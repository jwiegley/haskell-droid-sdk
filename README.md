...
Creation configuration includes caller session/workspace identity, legacy/interaction/autonomy/reasoning/spec settings, mission settings/decomposition metadata, tool policy, source/origin/location, tags, title/privacy, compaction checking, explicit permission-rejection and builtin-skill flags, raw `systemPromptOverride`, and nullable `structuredOutputFormat`. Raw configuration codecs preserve omission and explicit false/empty values; high-level producer attribution is applied as described below. Initial spec strings cannot use update-patch null clearing. Structured-output capability omission, explicit null and an object schema remain distinct; this does not evaluate JSON Schema keywords locally.

Local `droidMachineId` / `droidSessionMachineId` are optional, defaulting to `default`; an explicit empty string is not replaced. A requested session ID must match the initialization reply before a handle is exposed. Configuration extensions cannot override reserved identity, token, model, MCP or other declared fields.

High-level local and daemon creation append one canonical `sdk` tag with `language = "haskell"` and the compiled Cabal package version. Other tags retain their order and extensions, but caller-supplied exact `sdk` tags are replaced; even an explicit empty tag list receives the producer tag. This reserves library identity without changing the literal codecs or named low-level Client initialization calls.

High-level creation and public resume default `sessionOriginHint` to `sdk`; high-level messages default `userMessageSource` to `sdk`. Explicit native origin, source and message-source overrides remain available. Local resume/replacement retains its SDK-origin default. Daemon public attachment supplies default API delegation/source only for that request; inferred attribution is not retained for automatic reloads, while genuinely explicit caller load intent remains. These defaults select the current SDK policy; frozen TypeScript differences are recorded separately.

REST requests identify the producer with `X-Factory-Client: sdk` and `X-Factory-Sdk: haskell/<package-version>`. This does not grant authentication or prove backend acceptance of that identity. The approved omission policy still applies to the closed structured SDK-language/request-attribution domain, which has only Python and TypeScript values: native code does not add a Haskell enum member, impersonate either SDK, or inject unsupported process-environment attribution. Session-tag metadata is independently open and supports truthful Haskell strings. See [attribution verification and limits](docs/evidence/2026-09-17/attribution/README.md).
...
Repair preserves the first parent for duplicate IDs, retains unresolved parents and cuts the oldest member of each cycle, excluding incoming acyclic tails. The last duplicate body supplies its graph timestamp. Standalone ordering follows the newest leaf's chain, then appends disconnected history sorted by timestamp and ID; unlinked histories use that same key. Repair preserves input entries and their payloads apart from parent changes; linked ordering follows the SDK's duplicate-ID lookup semantics rather than promising duplicate-copy retention.

Loaded-message merging keeps a separate policy: a single rooted graph uses parent-chain ordering, while multiple roots retain stable timestamp order, as the current manager does. The new cycle-root and standalone tie rules supersede the frozen 0.7 helper policy. Native ID comparisons remain portable UTF-16 rather than TypeScript's host-locale `localeCompare`; case/accent-sensitive ties can therefore differ. Timestamps remain exact native numbers. See [current ordering evidence](docs/evidence/2026-09-17/cc06/README.md).
...
Tool names are opaque protocol strings and are preserved exactly, including whitespace, Unicode and empty strings. MCP's recommended 1–128-character ASCII spelling (`A–Z`, `a–z`, digits, `.`, `_`, `-`) remains the sensible interoperability choice, but is not enforced as a constructor admission rule. Duplicate names remain errors; distinct case, padding and Unicode normalization forms do not alias. HTTP authentication, payload bounds and schema checks are unchanged. The pure constructors do not emit the reference SDK's console warnings. Their `Either` return types and `InvalidMcpToolName` constructor remain for source compatibility, although name construction no longer returns that error. See [FC-06 verification and limits](docs/evidence/2026-09-17/mcp-names/README.md).
