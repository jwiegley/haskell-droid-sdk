...
| Existing-daemon WebSocket connection and authentication | `Factory.Droid.Transport.WebSocket` and `Factory.Droid.Daemon` implemented: explicit credentials, verified TLS by default, bounded setup/messages/cleanup; no automatic reconnect | `test/WebSocketSpec.hs`, `test/DaemonSpec.hs`: offline plain/TLS peers, trust/hostname rejection, framing, authentication, deadlines, cancellation and disconnect |
| Relay, binary tunnels and host message-channel transports | Pending; not supplied by the text-object WebSocket transport | Transport-specific authentication, framing and recovery tests remain required |
...
| One-shot execution and persistent sessions | `Factory.Droid` implements local new/resumed scopes and one-shot helpers; `Factory.Droid.Daemon` implements new/resumed existing-daemon scopes and follow-up turns | `test/DroidSpec.hs`, `test/DaemonSpec.hs`; historical live local single-turn evidence only |
...
| Complete/partial streaming, messages, usage and outcomes | Local `sendDroidEvents` and daemon `sendEvents` share complete/all-event modes, typed known events, raw unadapted notifications and result accumulation. Text-turn APIs retain outcomes; `sendPrompt` checks success | Native event order/filtering, snapshots/retractions, partial terminal text, raw/malformed payloads and callback failures; daemon session/turn-ID isolation and ACK ordering. Rich live interoperability remains unverified |
| Explicit interruption | Local `interruptDroidSession` and daemon `interruptSession` wait for submission and fence following turns; settled interruption preserves usability | Native idle/callback/retry/cancellation tests and local ordering mutants; daemon session-ID, partial-result and reuse assertions. Live verification remains required |
| Resume, fork, compaction, rewind and ownership transfer | Local resume, rewind-info and successor-producing controls retain retirement/rollback. Daemon resume is implemented with restored pending interactions; daemon fork/compact/rewind controls remain pending | Native local lifecycle/gate/trace tests and daemon load/handler/connection cleanup; no live replacement or daemon interoperability claim |
| Settings, system prompts, spec mode, context statistics and detailed context breakdown | Local typed updates/context queries, new-session prompts and `getDroidSettings` are implemented, with ordered local settings observations. Daemon init/load replies reuse full settings validation; daemon control/getter wrappers and ongoing settings-view reduction remain pending | Native local startup/resume/reset, callback read, malformed recovery and replacement checks; daemon init/load settings validation. Broader configuration and live verification remain pending |
...
| Images, documents and structured output | `Factory.Droid.Input` validates image/text/PDF values; local and daemon input/output APIs use the same turn engine, explicit object schemas and raw or `FromJSON` output adaptation. Frame limits remain configurable | Native source/size/signature/metadata/UTF-8/Base64 and output tests; daemon wire attachment/structured-output fixture. Existing actual-source attachment FD/race evidence is retained. No full image/PDF parser, JSON Schema validator, automatic schema derivation or new live verification |
| Permissions, typed actions and user questions | `Factory.Droid.Interaction` and new/resumed local/daemon handler scopes share validated adapters and safe defaults. Daemon permissions admit owned/associated execution sessions; questions admit only the owned session | Native adapter/privacy/lifetime tests; daemon restored-pending interactions, positive scope routing, unrelated-request cancellation and owned-worker finalization. The isolation regression fails before the fix; live interaction verification remains pending |
...
| Custom models, SSH keys, relay and update controls | Pending | Daemon resource tests remain required |
...
| `daemon.initialize_session` | `withSession` creates a UUID-addressed session with daemon-host cwd/machine, optional model and safe permission policy. The full reply is validated and must identify the requested session. |
| `daemon.load_session` | `withResumedSession` loads all messages and restores pending permissions/questions through normal dispatcher workers. Explicit resume model overrides fail before connection rather than being ignored. |
