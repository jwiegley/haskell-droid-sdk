...
| Haskell attribution | The approved omission of unsupported closed SDK-language metadata remains. High-level creation now canonicalizes a truthful Haskell/package-version tag, SDK source defaults are explicit, and REST sends truthful SDK identity headers. Raw codecs and explicit native source/origin overrides remain available. | See [current attribution evidence](evidence/2026-09-17/attribution/README.md). No Haskell member is invented in the pinned structured-language union; no TypeScript/Python impersonation or new backend/live acceptance is claimed. |
