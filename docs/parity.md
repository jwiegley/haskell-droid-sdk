...
| Child discovery, linkage and invocation hydration | Typed availability/load metadata, parent/tool lookup, provisional registration/cwd inheritance, configurable hydration and explicit ensure/summary APIs reuse existing owners | Native/source checks cover permission isolation, stale-load/close retirement, cancellation, first-registration lookup, ordered callbacks and provisional-cwd authority. Summary-only hydration does not register or execute children; turn completion does not invent terminal task status |
