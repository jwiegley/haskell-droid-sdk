...
- **Next:** `hsdk-25l6`, logical state and observer continuity across physical connection generations, still within `complete-reconnect-observability`. SLI delivery does not prove this other half. There are **5/12 verified milestones**; this is not an SDK completeness percentage. `hsdk-8b6` remains on the later native-attribution contract work.
- Remaining milestones: reconnect continuity (SLI unit delivered); ten current-only operations; current state contracts; remaining native policy decisions; original/repaired claim verification; packaged release validation; final frozen/current assessments.
