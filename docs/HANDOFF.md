...
- **CC-17 / `hsdk-9yl9`: retained-option administration delivered.** Logical-owner set/get/delete reuse the existing policy pair and distinguish replacement, empty memo and independent forgetting. Load admission captures defaults for replay. Three reproduced creation-rollback failures are fenced by per-entry policy revisions; the shared request validator also rejects non-true Task-process hints before RPC. Seventeen new cases, 84 focused checks, full 4,305-test suites and one public example pass on macOS/GNU/Linux ARM64; twenty final focused parallel repetitions pass on macOS. Ten memo/registration rows and two ID controls match actual current manager components; six actual controller-prefix admission cases stop at an inert transport boundary. See [CC-17 evidence and fess](evidence/2026-09-17/cc17/README.md).
- **FC-06 / `hsdk-evgq`: MCP name admission repaired.** The existing tool constructor now accepts opaque protocol strings rather than enforcing an advisory ASCII/nonempty/128-character naming style. Public `Either` signatures and the legacy error constructor remain compatible; no global warning output or replacement registry was introduced. Twenty new cases failed 17/20 before the change. Final 93-case hosted checks and full 4,325-test suites pass on macOS/GNU/Linux ARM64; fifteen actual HTTP name/routing/result projections match both native platforms and both pinned reference SDKs. Server/security/schema ownership is unchanged. See [MCP-name evidence and fess](evidence/2026-09-17/mcp-names/README.md).
- **Attribution / `hsdk-8b6`: truthful producer defaults delivered.** Creation canonicalizes Haskell/Cabal-version SDK tags, high-level origins/message sources select SDK defaults with explicit native overrides, and REST adds truthful identity headers. Public daemon attachment attribution remains separate from automatic retained intent; local replacement follows its distinct current SDK-origin policy. The closed Python/TypeScript structured-language domain and environment sanitization remain unchanged, without impersonation or backend acceptance claims. Twenty-one new cases and all thirteen REST helpers are checked; final 4,346-test suites pass on macOS/GNU/Linux ARM64. Earlier reference-helper and broad shared-fixture failures are retained, with exact corrected expectations. See [attribution evidence and fess](evidence/2026-09-17/attribution/README.md).
- **Message omission / `hsdk-c9j4`: observed-metadata retention repaired.** The candidate was reproduced: native updates lost hidden/visibility/hook/opaque metadata that both references retained. The existing `observeCreatedMessage` boundary now preserves omissions with typed fields while explicit values win; direct full-record edits, authoritative loads and codecs remain unchanged. Seven of twelve regressions failed before repair. Final 98-case focused and 4,358-test full suites pass on macOS/GNU/Linux ARM64; all 33 current component snapshots across twelve sequences match both native platforms. Frozen non-parent values/display projections agree, with three declared CC-16 ancestry differences retained. See [message-omission evidence and fess](evidence/2026-09-17/message-omission/README.md).
- **FC-01/02/03 / `hsdk-2nha`: lifecycle and turn contracts calibrated.** Borrowed scopes retain explicit remote-close ownership, with fresh typed sequential-close evidence for normal/error/cancel paths and visible rejection. SDK stream waits now observe connection closure; already-running caller callback IO remains caller-owned. Overlapping and nested prompts fail fast through the existing gate, without a second submission or incumbent invalidation; caller-side serialization remains available. Five of ten new cases failed before the repair. Final 4,368-test suites pass on macOS/GNU/Linux ARM64; twenty final parallel repetitions pass. The old replacement expectation and a parallel test-peer publication race were corrected and retained as failures. See [lifecycle/turn evidence and fess](evidence/2026-09-17/lifecycle-turns/README.md).
- **Next:** `verify-sdk-claims`. Native-contract calibration is now verified, bringing the program to **9/12 verified milestones** after the local commit/task update; this is not an SDK completeness percentage. `hsdk-w50g` and `hsdk-g6o9` require original-source/claim reconciliation. Preserve all original denominators and prior contradictions while separately checking repaired/current promises.
- Remaining milestones: original/repaired claim verification; packaged release validation; final frozen/current assessments.
