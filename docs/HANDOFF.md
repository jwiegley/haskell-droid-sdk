...
The latest request, “Continue and complete those two separate jobs remaining,” supersedes the earlier read-only/no-fixes boundary. Implement the required repairs and finish verification, working sequentially with local commits. The confirmed goal tree has twelve milestones: the two original comparisons are complete, and the policy/process and MCP repair milestones are now verified. Any old read-only wording in archived goal/report text is historical; it does not override this scope amendment. Within the original four-part audit, the position remains:
...
- **FC-07 / `hsdk-laj`: existing-owner creation delivered.** `DaemonSessionOptions`, `withSessionOn` and its handlers variant reuse the existing authentication, initialization, readiness and attachment owners; legacy creation delegates to the same initializer. Fourteen new cases cover shared/concurrent scopes, exact fields and fresh credentials, duplicate/invalid admission, rejected/malformed receipts, cancellation, stale-provider fencing, early handlers, retry and future-load policy. The original public-API probe failed to compile; afterward 37 initialization checks and full 4,034-test suites pass on macOS/GNU/Linux ARM64, plus ten parallel repetitions of the fourteen new cases. The owner milestone remains open: cache controls and provisional-state retirement are next, not certified by constructor tests. See [FC-07 evidence and fess](evidence/2026-09-16/fc07/README.md).
- **Next:** continue `complete-daemon-owner` with `hsdk-0aq` (eligible cache capacity/recency/removal and coherent retirement), then local directory/machine administration. There are still **4/12 complete milestones**.
- Remaining milestones: daemon cache/directory; reconnect/SLI; ten current-only operations; current state contracts; remaining native policy decisions; original/repaired claim verification; packaged release validation; final frozen/current assessments.
...
- **FC-07, original release:** no integrated public creation operation was available on the opaque active daemon owner. The new scoped creation API above supplies that operation. Cache administration, provisional-state retirement, directory/machine and retained-policy administration, and specialized SLI semantics still require their remaining work. Generic RPC or unrelated application copies do not automatically satisfy them.
...
> Read `docs/HANDOFF.md`, `~/Products/droid-sdk-haskell/completion-20260916/state.json` and the confirmed goal tasks. Continue the authorized SDK repair and verification program, starting with the next open owner-milestone item. Keep the original audit corpus and pinned references fixed. Use existing owners and public Haskell composition; do not introduce an unused-schema quota, unrelated cleanup, reviewer waves or redundant live calls. Finish all required capability, claim and release verification. Run `fess` after every subtask, preserve failures, commit locally, and do not push.
