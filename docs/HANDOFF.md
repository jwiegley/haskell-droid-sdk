...
- **Next:** FC-11 MCP timestamp grammar, then FC-12 content annotations, within the active `repair-mcp-contracts` milestone. The policy/process milestone is complete.
...
- **FC-11/12:** MCP timestamp grammar and Draft 7 content-annotation policy still differ from the evaluated reference behavior. The annotation discrepancy changes actual handler admission, including under negation.
