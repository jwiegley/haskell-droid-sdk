...
- **Next:** continue `resolve-native-contracts`: lifecycle, caller-owned pending work and overlap calibration remain. The program is at **8/12 verified milestones**, not an SDK completeness percentage. `hsdk-w50g` and `hsdk-g6o9` record original-claim reconciliations for the verification stage.
- Remaining milestones: native policy decisions; original/repaired claim verification; packaged release validation; final frozen/current assessments.
