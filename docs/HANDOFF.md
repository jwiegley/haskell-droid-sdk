...
- **Next:** FC-12 content annotations, the remaining item in `repair-mcp-contracts`. The policy/process milestone is complete.
...
- **FC-12:** Draft 7 content-annotation policy still differs from the evaluated Python behavior, changing actual handler admission including under negation.
