...
The latest request, “Continue and complete those two separate jobs remaining,” supersedes the earlier read-only/no-fixes boundary. Implement the required repairs and finish verification, working sequentially with local commits. The confirmed goal tree has twelve milestones: the two original comparisons, policy/process repairs, MCP repairs and existing-owner creation/cache/directory work are now verified. Any old read-only wording in archived goal/report text is historical; it does not override this scope amendment. Within the original four-part audit, the position remains:
...
- **Directory / `hsdk-dko`: delivered.** Registration, machine association, atomic directory snapshots/STM observation, machine membership/cwd work counts and scoped invalidation reuse the existing cache/readiness owners. Ten new cases cover empty/unknown values, pre-init and optimistic inherited cwd, reassignment, cache membership, observers, malformed state and cross-machine/superseding loads. Actual frozen/current reference programs agree on eight directory and four optimistic snapshots. Final 4,061-test suites pass on macOS/GNU/Linux ARM64, with twenty parallel repetitions and two public examples compiled on both. The initial invalid-null-cwd fixture failure remains recorded; the wire codec was not relaxed. See [directory evidence and fess](evidence/2026-09-16/directory/README.md).
- **Next:** `complete-reconnect-observability`, covering shared-state continuity across connection generations and the specialized computer-connect SLI contract. There are now **5/12 verified milestones**; this is not an SDK completeness percentage. `hsdk-8b6` remains on the later native-attribution contract work.
- Remaining milestones: reconnect/SLI; ten current-only operations; current state contracts; remaining native policy decisions; original/repaired claim verification; packaged release validation; final frozen/current assessments.
...
- **FC-07/08 and integrated directory, original release:** public creation on an active opaque daemon owner, cache administration and machine-group administration were absent or partial. The new owner APIs above provide these operations, including guarded provisional-state retirement. Cross-generation continuity, retained-policy administration, attribution decisions and specialized SLI semantics still require their remaining work. Generic RPC or unrelated application copies do not automatically satisfy them.
