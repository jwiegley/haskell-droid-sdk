...
- **Next:** continue `complete-daemon-owner` with `hsdk-dko`, local directory/machine association, group counts and scoped invalidation. There are still **4/12 complete milestones**. `hsdk-8b6` remains on the later native-attribution contract work.
- Remaining milestones: daemon directory/machine administration; reconnect/SLI; ten current-only operations; current state contracts; remaining native policy decisions; original/repaired claim verification; packaged release validation; final frozen/current assessments.
...
- **FC-07/08, original release:** public creation on an active opaque daemon owner and integrated cache administration were absent. The new scoped creation and cache APIs above provide these operations, including guarded provisional-state retirement. Directory/machine and retained-policy administration, attribution decisions, and specialized SLI semantics still require their remaining work. Generic RPC or unrelated application copies do not automatically satisfy them.
